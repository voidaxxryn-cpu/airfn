...
namespace utils
{
	std::wstring GetFullTempPath()
	{
		...
		const DWORD get_temp_path_ret = GetTempPathW(MAX_PATH + 1, temp_directory);
		if (!get_temp_path_ret || get_temp_path_ret > MAX_PATH)
			return L"";
		auto len = wcslen(temp_directory);
		if (len > 0 && temp_directory[len - 1] == L'\\')
			temp_directory[len - 1] = L'\0';
	...
	uint64_t GetKernelModuleAddress(const std::string& module_name)
	{
		...
		{
			const std::string current_module_name = reinterpret_cast<char*>(modules->Modules[i].FullPathName) + modules->Modules[i].OffsetToFileName;
