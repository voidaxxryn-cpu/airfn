...
ULONG64 kdmapper::MapDriver(BYTE* data, ULONG64 param1, ULONG64 param2, bool free, bool remove_from_system_page_tables, bool destroyHeader, AllocationMode mode, bool PassAllocationAddressAsFirstParam, bool PassSizeAsSecondParam, mapCallback callback, NTSTATUS* exitCode) {
	(void)remove_from_system_page_tables;
	...
	else if (mode == AllocationMode::AllocateContiguousMemory) {
		kernel_image_base = intel_driver::AllocContiguousMemory(image_size);
	}
	...
	do {
		...
		ULONG64 entry_param1 = PassAllocationAddressAsFirstParam ? realBase : param1;
		ULONG64 entry_param2 = PassSizeAsSecondParam ? image_size : param2;

		if (callback) {
			if (!callback(&entry_param1, &entry_param2, realBase, image_size)) {
		...
		NTSTATUS status = 0;
		if (!intel_driver::CallKernelFunction(&status, address_of_entry_point, entry_param1, entry_param2)) {
		...
		if (free) {
			...
			if (mode == AllocationMode::AllocateIndependentPages)
			{
				...
				free_status = intel_driver::FreeContiguousMemory(realBase);
			}
			else {
	...
	if (free_status) {
		...
		free_status = intel_driver::FreeContiguousMemory(kernel_image_base);
	}
	else {
		Log(L"[-] WARNING: Failed to free memory!" << std::endl);
	}
